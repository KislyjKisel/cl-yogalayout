...
; (sb-int:set-floating-point-modes :traps '()) ; defun fails, even if ran "with traps masked", when using +UNDEFINED+ to call function
; (defun node-child-test ()
...
;       (setf result (and result (equal 0.0 (yogalayout:node-layout-get-left root-child0))))
;       (setf result (and result (equal 0.0 (yogalayout:node-layout-get-top root-child0))))
;       (setf result (and result (equal 100.0 (yogalayout:node-layout-get-width root-child0))))
;       (setf result (and result (equal 100.0 (yogalayout:node-layout-get-height root-child0))))
;       (yogalayout:node-remove-child root root-child0)
;       (setf result (and result (equal 0.0 (yogalayout:node-layout-get-left root-child0))))
;       (setf result (and result (equal 0.0 (yogalayout:node-layout-get-top root-child0))))
;       (setf result (and result (yogalayout:float-is-undefined (yogalayout:node-layout-get-width root-child0))))
;       (setf result (and result (yogalayout:float-is-undefined (yogalayout:node-layout-get-height root-child0))))
